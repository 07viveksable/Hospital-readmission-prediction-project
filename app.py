...
# Main content
if model is None:
    st.error("⚠️ Model not loaded. Please ensure model files are in directory.")
else:
    # Create tabs
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader("diabetic_data", type="csv")
        ...
        with col2:
            ...
            try:
                ...
                if hasattr(model, 'feature_importances_'):
                    # Get feature names from model_info
                    if model_info and 'features' in model_info:
                        feature_names = model_info['features']
                    else:
                        # Default feature names if not in model_info
                        feature_names = [
                            'time_in_hospital', 'num_lab_procedures', 'num_procedures',
                            'num_medications', 'number_outpatient', 'number_emergency',
                            'number_inpatient', 'number_diagnoses', 'age_numeric',
                            'gender_encoded', 'medication_changed', 'diabetic_med_prescribed',
                            'total_visits', 'had_emergency'
                        ]
                    ...
                    feature_imp = pd.DataFrame({
                        'Feature': feature_names[:len(importances)],  # Match length
